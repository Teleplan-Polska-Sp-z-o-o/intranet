...
export namespace EfficiencyModels {
  ...
  export class EfficiencyBuilder {
    ...
    constructor(
      rawTransactions: RawTransactions.TTransactionsDellBose,
      models: EfficiencyTypes.IModelRecord[]
    ) {
    ...
    private buildModelsCache(models: EfficiencyTypes.IModelRecord[]) {
      models.forEach((model) => {
        const key = `${model.WORKSTATION_DESC}`;
        const value = parseFloat(model.TT);
        this.models.set(key, value);
      });
    }

    // --- Main method to process the raw transactions ---
    private processTransactions(transactions: RawTransactions.TTransactionsDellBose) {
      ...
      transactions.forEach(
        (
          ...
          const { username, workStationDesc, lastActivityDate } = transaction;
          const key = `${workStationDesc}`;
          const processingTime = this.models.get(key);
          if (!processingTime) {
            console.error(`No model data for workstation ${workStationDesc}`);
            return;
          }

          const transactionDate = this.extractTransactionDate(lastActivityDate);
          const transactionQuarter = this.extractTransactionQuarter(lastActivityDate);
          const shift = this.extractShift(lastActivityDate);

          this.initializeEmployeeData(employeeDataMap, employeeWorkedQuarters, username, shift);
    ...
    private updateEstimatedTargets(
      ...
      const key = `${work_center_no}`;
      if (!estimated_target.units[key]) {
        estimated_target.units[key] = 0;
      }
      // Increment the count of the specific workstation
    ...
    private calculateWeightedAverages(employee: EfficiencyTypes.IProcessedEmployee) {
      ...
      for (const key in units) {
        ...
        totalProcessingTime += processingTime * unitsCount;
    ...
    private update(
      ...
      this.updateEstimatedTargets(employeeDataMap[emp_name].estimated_target, work_center_no);
