...
import { Bose, TouchTimeUnit } from "../../Models/models/TouchTime";
...
export class BoseEfficiencyBuilder<T extends Bose.TTS> {
  ...
  constructor(
    ...
    this.models = new Models(
      modelsMatrix,
      (model: T) => {
        const workCenter = model["WORK_CENTER"];
        const processType = model["PROCESS_TYPE"];
        const family = model["FAMILY"];
        return `${workCenter} ${processType} ${family}`;
      },
      touchTimeKey
    );
  ...
  private update(
    ...
    const modelData = this.models.getTT(`${workCenterNo} ${processType} ${family}`);
    if (!modelData) return;
