...
const editNotice = async (req: Request, res: Response) => {
  try {
    ...
    const fields: Partial<IProcessChangeNoticeFields> = JSON.parse(body.fields);
...
//         notice = await transactionalEntityManager
//           .getRepository(ProcessChangeNotice)
//           .save(notice.assessByDepartment(user.info.department.id, assessment));
//       }
//     });
...
export { editNotice, closeNotice, getNotices };
