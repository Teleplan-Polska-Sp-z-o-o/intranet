...
const nameRules = computed(() => [
  ...
  (value: string) => {
    if (/^[a-zA-Z]+\.[a-zA-Z]+\d*$/.test(value)) return true;
    return "Please enter the login in the format: 'name.surname'";
  },
  // /^[a-zA-Z]+\.[a-zA-Z]+$/
]);
const domainRules = computed(() => [(value: string) => !!value || "Domain is required."]);
    ...
    <v-row>
      <v-col class="h-screen ma-n2 d-flex flex-column justify-center">
        <v-card
          ...
          class="bg-surface-1 h-auto d-flex flex-column align-center mx-auto rounded-xl"
        >
          <template v-slot:loader="{ isActive }">
            <v-progress-linear
              :active="isActive"
