...
const processLoginDetails = (entities: Array<IUserLoginDetails>): Array<number> => {
  ...
  const averageDurations: number[] = Array(7).fill(0);
  const loginCounts: number[] = Array(7).fill(0);
  ...
  entities.forEach((detail) => {
    ...
    if (dayIndex !== -1) {
      averageDurations[dayIndex] += detail.duration / 60;
  ...
  for (let i = 0; i < 7; i++) {
    if (loginCounts[i] > 0) {
      averageDurations[i] /= loginCounts[i];
    }
  }
...
const data = computed<{ labels: Array<string>; datasets: Array<ChartData> }>(() => {
  return {
    ...
    datasets: [new ChartData("Average Duration", datasets.value, chartHelper)],
  };
});
...
<template>
  <v-card class="rounded-xl elevation-2">
    <v-card-text>
      ...
      <v-card-subtitle> Average session duration (minutes) over the past week </v-card-subtitle>
