...
watch(
  ...
  async (newRawTransactions: AnalyticRaw.TTransactions) => {
    ...
    function hasTTCleaningProperty(
      model: EfficiencyTypes.IModelObj
    ): model is EfficiencyTypes.TCleaningModelObj {
      return "TT_CLEANING" in model;
    }

    const cleaningModelsObj = unref(modelsObj).filter(
      hasTTCleaningProperty
    ) as EfficiencyTypes.TCleaningModelObj[];

    if (cleaningModelsObj.length === 0) {
      throw new Error("No models found with the 'TT_CLEANING' property");
    }
...
<template>
  <v-card class="bg-surface-2 pa-4 ma-1 rounded-xl elevation-2">
    <v-card-title class="d-flex align-center">
      {{ title ?? "Employee Cleaning Efficiency Overview" }}
      <v-spacer></v-spacer>
      <download
