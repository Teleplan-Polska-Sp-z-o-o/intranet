...
const test = (value: unknown, key: string) => {
  ...
  if (typeof value === "string" && key === "fileNames") {
...
const formatValue = (value: unknown, key: string) => {
  ...
  switch (typeof value) {
    ...
    case "string":
      ...
      if (key === "fileNames") {
        return props.files.map((file) => file.file?.at(0)?.name).join(", ");
      }
      if (key === "tags") {
...
const disallowedKeys = ["docxSource", "dialog"];
</script>
...
<template>
  <v-sheet border class="rounded-xl mb-4">
    <template v-for="(value, key, index) in props.documentChangeFields" :key="key">
      <v-card
        v-if="value && test(value, key as string) && !disallowedKeys.includes(key as string)"
        color="transparent"
        elevation="0"
        outlined="false"
        class="pa-2"
